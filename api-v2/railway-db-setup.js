...
async function executeSqlFile(client, filePath, fileName) {
  try {
    ...
    // Fix common issues for Railway deployment
    sql = sql.replace(/CREATE SCHEMA public AUTHORIZATION [^;]+;/g, '-- Schema already exists, skipping');
    sql = sql.replace(/CREATE SEQUENCE public\./g, 'CREATE SEQUENCE IF NOT EXISTS public.');
    ...
    if (error.message.includes('already exists') ||
        ...
        error.message.includes('already has a role')) {
      console.log(`   ⚠️  Continuing despite duplicate objects...`);
      return true;
    }

    // Log but continue for migration errors (might already be applied)
    if (fileName.includes('Migration')) {
      console.log(`   ⚠️  Migration error (might already be applied), continuing...`);
      return true;
    }
